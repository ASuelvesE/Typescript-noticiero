    ...
    async save(noticia: Noticia): Promise<Noticia[]> {
        ...
        try{
            ...
            noticia.recursos.forEach(async (recurso) => {
                ...
                const sql3:string = `insert into noticiasRecursos values (${noticia.id},${recurso.id})`
                await executeQuery<Noticia[]>(sql3)
            });
            noticia.periodistas.forEach(async (periodista) => {
                const sql4:string = `insert into noticiasPeriodistas values (${noticia.id},${periodista.id})`
